# v7.3 Better filename handling
# For raw files:
#   Assumes the filename is WPI_GROUP_TEAMNUMBER_{the rest here}_raw.csv
#     where WPI_GROUP_TEAMNUMBER_ is unique to this file.
# 1) WPI, GROUP, and TEAMNUMBER can be multiple characters now, which is cool. 
# 2) I discovered that the "smarter" way to interpolate missing y-values in version 7.2 
#       caused probably as many errors as the original method, but the errors were no longer
#       as predictable. It felt misleading, so I'm returning to this method.
# 3) There was a bug in detect_direction_with_flow.py where I calculated the maximum and minimum
#       drift speeds (flow-adjusted "static"). I fixed the bug and changed this script accordingly.

# v7.2  Speedups, refactoring, and alignment bugfix
#    1) Significantly faster alignment.
#         You're welcome.
#    2) Column names have no assumed order for summary_full and summary_avg.
#         Improves maintainability, easier to add future features.
#    3) Alignment no longer copies over y-values above EXHAUSTION_BOUNDARY - 1
#         I chose to do this because we don't want an invisible fish to show a perpetually high y-value.
#         If the fish is not visible for 100 frames, we can assume it is exhausted, and cannot assume it has
#         been swimming at some previously high y-position for the whole time without being observed.
#         So, please realign all files to get this fix. Y-positions will be a little different, but I trust them more.
#         Since aligning is super fast now, I'm not shy about asking you to do this.

# v7 (7.0, 7.1) (Nick Jensen) added two features:
#    1) (v7.1) Activity and "Time Against Flow" are adjusted according to flow speed.
#         If water is flowing, then treading water in the same y-position is still moving forward
#         unless the fish is exhausted and resting at the very back.
#    2) (v7.0) summarize y-position by quadrant (% of frames spent in each y quadrant)
#    3) fixed a bug when calculating mean amplitudes
#    4) fixed a bug where burst frequency was reported as the total number of bursts in the 5 minute period.
#             Now it reports bursts/min as named in the SummaryData_Full.csv file.
#    5) Removed certain places where the first line was being omitted from analysis using [-1] indexing.
#             I think the [-1] indexing operator was not understood by the author.
#    6) Changed all output from jpeg to png
#           Plots are sharper (no jpeg compression) and use less space.

# Note: use rlang::last_error() and rlang::last_trace() while debugging.
#         It will save you lots of trouble.

# v7 (Nick Jensen) enforced a more readable and maintainable coding style:
#    -1) start with a style guide or a linter, like these:
#          https://google.github.io/styleguide/Rguide.html
#          https://github.com/jimhester/lintr
#    0) Basically all code should be contained inside functions.
#    1) use whitespace to surround all assignment
#          and equality operators (== and <-)
#    2) Never use reserved words like "filter" or "sum".
#          These words are already defined in the language,
#             and redefining them will cause headaches.
#          For example, I renamed the boolean variable "filter" to "do_filter".
#    3) To check a boolean, do not use `if (do_filter == T)`.
#          Use `if (do_filter)`. It is just so much simpler.
#    4) The correct way to check if a variable is false is to use
#          `if (!do_filter)`, not `if (do_filter == F)`
#    5) There should be space between "if" and the open paren "(". Use `if (...`
#    6) There should be space after every comma
#    7) There should be space between "}" and "("
#    8) There should be space around every operator, like " + "
#    9) Within a single scope, never ever ever
#          ever EVER use the same variable name for multiple things.
#          If you already had a "myFiles" variable holding a list of files,
#          do not reassign it with entirely unrelated list of files later on.
#   10) Global variables should be UPPER_UNDERSCORE_CASE
#   11) All `if ... else ...` blocks should not all be on one line.
#   12) Try to keep your lines shorter than the width of your screen. Do use
#           line breaks as appropriate.
#   13) A function should rarely, if ever, be longer than the height of your screen.
#   14) A function should rarely, if ever, contain more than eight logical blocks (e.g., `if`, `for`, `while`)
#   15) If you use an imported library function, please include the namespace to avoid collisions.
#          For example, `stri_flatten` should be `stringi::stri_flatten`

# DEBUGGING NOTES FOR VS CODE:
# Install vscDebugger. In August 2021, the commands below do the trick:
#  >> packageurl <- "https://github.com/ManuelHentschel/vscDebugger/releases/download/v0.4.7/vscDebugger_0.4.7.tar.gz"
#  >> install.packages(packageurl, repos=NULL, type="source")
# Also install "languageserver" and "jsonlite"
# Also install "rowr" using a URL (rowr is no longer maintained)
#  >> packageurl <- "https://cran.r-project.org/src/contrib/Archive/rowr/rowr_1.1.3.tar.gz"
#  >> install.packages(packageurl, repos=NULL, type="source")

### PREVIOUS AUTHOR NOTES:
#v6 Was created to improve the tracking algorithm.
#    I tried a number of different fixes (v6-1, 2, 3, 4, 5), but eventually
#    landed on v6-6 as the best (and nearly perfect) tracker

#V6-7 I included a filter to remove non-swimmers.
#    This filtering is based on inactivity during the 10 cm/s swim (currently, fish who are
#    inactive in >50% of 10 cm/s swim are excluded).
#NOTE: Because tracks can sometimes be swapped between fish, we cannot say with complete certainty
#      that excluding fish based on swim behavior at a certain time point will
#      completely exclude those fish from the analysis.
#v6-6 A tracking fix was implemented in which, while there are missing objects,
#      whenever the number of missing objects decreases, the missing objects
#      with the lowest Y values are maintained.
#I also included a quality control prompt to check how many objects are in each
#      raw tracking file before proceeding with the analysis.
